...
app.post('/api/presupuestos', verificarToken, async (req, res) => {
    ...
    // Cálculo del límite diario (30 días para mensual, 7 para semanal)
    const dias = periodo.toLowerCase() === 'semanal' ? 7 : 30;
    const limiteDiarioCalculado = dineroDisponibleTotal / dias;

    console.log("Valores calculados:", {
        disponible: dineroDisponibleTotal,
        limite: limiteDiarioCalculado
    });

    try {
        // 2. Guardar en Supabase (Usamos Upsert para crear o actualizar)
        const { data, error } = await supabase
            .from('Presupuesto')
            .upsert({
